...
static void quickSort(void* p) {
    ...
    if (j - i > SORT_THRESHOLD) {           /* if the sort range is substantial, use quick sort */
        ...
        int m = rand() % (j - i + 1) + i;               /* pick pivot as median of         */
        ...
        if(currentThreads < maximumThreads){ //make new thread left
            ...
            if(threadCheck != 0) fprintf(stderr, "%s\n", strerror(errno));
        ...
        if(currentThreads < maximumThreads){ //make new thread right
            ...
            threadCheck = pthread_mutex_lock(&mutex);
        ...
        if(createdLeft == 1) pthread_join(leftThread, NULL); //have to wait for it to finish

        if(createdRight == 1) pthread_join(rightThread, NULL); //have to wait for it to finish
