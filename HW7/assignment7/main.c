...
int main(int argc, char **argv) {
	const char *filename = "webster_random";
	char c, str[20];
	int wordCount = 0, i = 0;
	clock_t start, end;
...
//-------------------------------------TIME TO SORT-------------------------------------
	...
	sleep(1);
    int wrongtracker[wordCount];
	for(int i = 0; i < wordCount; i++) {
        printf("%s --- sorted wordArray2[%d]\n",wordArray2[i],i);
        if(strcmp(wordArray2[i],wordArray[i]) !=0){ sameCheck = 1; arraynum = i;
        wrongtracker[i]=arraynum;
        } else{wrongtracker[i] = 999;}
    }           
        if(sameCheck == 0){printf("sorted arrays are the same\n");}
        else if(sameCheck == 1){
            printf("sorted arrays are different\n");
            for(int i=0; i<wordCount; i++){
                printf("wrong at %d\n", wrongtracker[i]);
            }
        }
