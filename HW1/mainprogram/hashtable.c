...
void hashAdd(hashEntry *hashTable, char *wordpair, int hashsize){
    listAdd(hashTable[hash(wordpair, hashsize)], wordpair);
}
...
//creates a new hashTable with a magnitude of 3 larger hashsize and moves over previous hashtable before freeing previous hashtable.
// hashGrow(hashEntry *hashTable){
...
void listAdd(struct node *sent, char *pair){
    struct node *new = malloc(sizeof(struct node));
    char *newPair = malloc((strlen(pair)+1));
    strcpy(newPair, pair);

    //int newOccurrences = malloc(sizeof(int));
    int newOccurrences = 1;

    new->pair=newPair;
    new->occurrences=newOccurrences;
    new->next=sent->next;
    sent->next=new;
}
...
int listFind(struct node *sent, char *wordpair){
    ...
    while(current != NULL){
        if(strcmp(wordpair, current->pair) == 0){
            current->occurrences +=1; //not sure if this will work.
            return 1;
        }
        current = current->next;
    }
    //node with the same word pair was not found
    return 0;
}

//iterates through a linked list sent from the hash array and sums the number of non-sentinel nodes present.
//returns them to hashDump and bumps the contents of the list into an array for later qsort use.
