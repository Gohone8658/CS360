...
int lineNum(char *dictionaryName, char *word, int length) {
	...
	//If file can't be opened write errno and strerror to stderror, return errno from function.
	if(fd < 0){
	...
	while ((offsetLow != offsetHigh) && (offsetLow < offsetHigh)){
		...
		offsetCurrent = (((offsetLow + offsetHigh) / 2) / length) * length; // divide and multiply by length to get rid of offsetCurrent remainder (start at beginning of a line).
...
//If I were to start this assignment again I would turn manual error printing into a function
//and use the error printing function to perform checks that (errno == 0) after each read/write/open/close call to return errors.
